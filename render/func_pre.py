            ...
            if target_parent.type == 'ARMATURE' and target_bone:
                try:
                    ...
                    for scale_index in bone.scale:
                        if scale_index == 0:
                            print(f"ERR - {obj.name}: {bone.name} has a scale value of {scale_index}! Using {target_parent.name} instead.")
                            break
                    ...
                    print(f'Error Setting {obj.name} Parent: Does the "{target_bone}" bone exist on the "{target_parent}" armature?')
